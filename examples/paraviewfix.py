...
output_directory = Path(__file__).parent / "_outputs" / "paraviewfix"
...
segmentation = Roiable(filename=str(segmentation_path))
...
if not segmentation.isImageSet():
    raise RuntimeError("The segmentation was not loaded into Roiable.")
...
segmentation_output = segmentation.writeParaView(
    str(output_directory / "segmentation.vtp"),
    space="lps",
)
